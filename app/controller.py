...
@app.route('/admin/<coursename>/manageexams')
@login_required
def manageExams(coursename):
	...
	return render_template('admin_exams.html', exams=exams, coursename=coursename)

@app.route('/admin/<coursename>/manageexams/addexam')
@login_required
def addexam(coursename):
	if request.method == 'POST':
		...
		db.session.add(exam)
		db.session.commit()
		return redirect('manageExams', coursename=coursename)
	else:
		return render_template('manageExams', coursename=coursename)
